...
module.exports = function(fixtureConfig, mongoose, conn, rootCallback){
    ...
    var fixtureData = require(fixtureConfig.dataFixturePath+fixtureDataKey)(mongoose, conn, function(err, globalFixtureData){
        ...
        // then have
        var fixtureModel = null;
        // wait check name?
        //console.log(schemaKey.red);
        //console.log(fixtureItem.cyan);
        if(_.has(conn.models, fixtureItem)){
            fixtureModel = conn.models[fixtureItem];
            console.log('check '.red, fixtureModel);
        } else {
            fixtureModel = conn.model(fixtureItem, fixtureSchema);
            console.log('blah '.cyan, fixtureModel);
        }
