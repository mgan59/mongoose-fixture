...
var fixtureConfig = [];
    ...
    /* 
     * Pattern here with creating these varios boilerplates. consider adapting yeoman generator format
     */

    // TODO check if config file exists already and prompt for overwrite
    //  or make config name a parameter and assume client knows not to overwrite
    if(argv.generateConfig){
        ...
        var configFileName = 'mongoose-test-config.js';
...
// check if a boiler plate generator is being made
if(argv.generateFixture !== '' || argv.generateSchema !=='' || argv.generateConfig){
    ...
    if(argv.generateSchema !== ''){
        ...
        // setup our tag replacements
        var htmlReplacements = {'__schemaName__':schemaFileName,'__version__':npmPackage.version};
        //for key in htmlReplacements
        for(var key in htmlReplacements){
    ...
    // use config file to get schema/fixture locals
    
    // then load the boilerplate code from lib/
    // copy boilerplate into configFile/schema||fixture
