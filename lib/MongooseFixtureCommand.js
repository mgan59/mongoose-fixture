...
if(_.isArray(mongoSettings.servers)){
    ...
    _.each(mongoSettings.servers, function(setting){
        if(first === false){
            first = true;
            settingUrl += 'mongodb://'+setting.host+':'+setting.port+'/'+mongoSettings.dbname+','; 

        } else {
            settingUrl += 'mongodb://'+setting.host+':'+setting.port+','; 
        }
    });
    mongoConnectionString = settingUrl.substr(0,settingUrl.length-1);
    //'/'+mongoSettings.dbname;

// if no array then execute over the obj-literal to build a connection string
} else {
    mongoConnectionString = 'mongodb://'+mongoSettings.host+':'+mongoSettings.port+'/'+mongoSettings.dbname; 
...
/* Trying to use an entirely different mongoose connection for replica */
var db = mongoose.createConnection();
var ctr = 0;
db.openSet(mongoConnectionString, function(){
  ctr += 1;
console.log('Connected to mongo server'.mongoose);
console.log('check (',ctr,')',db);
  // instantiate an instance of mongoose-fixture 
  // to scope hoist mongoose and fixtureConfig listings
  var mongooseFixture = MongooseFixture(fixtureListing, db.base.connections[0]);
