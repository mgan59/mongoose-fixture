...
if(argv.generateFixture !== '' || argv.generateSchema !==''){
    ...
    if(argv.generateSchema !== ''){
        var fileSuffix = ((argv.suffix)? capitalizeFirstLetter(argv.suffix):'');
        // create file name, should add check to see if .js was passed in argument
        var schemaFileName = argv.generateSchema+fileSuffix+'.js';
        // caps first letter of schema
        var schemaModelName = capitalizeFirstLetter(argv.generateSchema);
        ...
        // setup our tag replacements
        var htmlReplacements = {
            '__schemaFileName__':schemaFileName,
            '__schemaName__':schemaModelName,
            '__version__':npmPackage.version
        };
