...
if(argv.generateFixture !== '' || argv.generateSchema !==''){
    ...
    if(argv.generateFixture !== ''){
        ...
        var htmlReplacements = [
            {
                pattern:/__dataFixtureName__/g,
                value:fixtureFileName
            },
            {
        ...
        //for key in htmlReplacements
        for(var ctr=0; ctr < htmlReplacements.length; ctr++){
            var tokenSet = htmlReplacements[ctr];
            fixtureSource = fixtureSource.replace(tokenSet.pattern,tokenSet.value);
        }
    ...
    if(argv.generateSchema !== ''){
        ...
        var htmlReplacements = [
            {
                pattern:/__schemaName__/g,
                value:argv.generateSchema
            },
            {
                pattern:/__schemaFileName__/g,
        ...
        for(var ctr=0; ctr < htmlReplacements.length; ctr++){
            var tokenSet = htmlReplacements[ctr];
            schemaSource = schemaSource.replace(tokenSet.pattern,tokenSet.value);
        }
...
if(argv.add === false && argv.reset === false && argv.remove === false){
    console.log('Required fixture action (--add|--reset|--remove) was not specified');
    process.exit();
}
